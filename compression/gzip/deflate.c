...
#define INSERT_STRING(s, match_head) \
   (UPDATE_HASH(ins_h, window[(s) + MIN_MATCH-1]), \
    prev[(s) & WMASK] = match_head = head_gzip[ins_h], \
    head_gzip[ins_h] = (s))
    ...
    ush *flags;     /* general purpose bit flag */
{
    ...
    for (j = 0;  j < HASH_SIZE; j++) head_gzip[j] = NIL;
#else
    memzero((char*)head_gzip, HASH_SIZE*sizeof(*head_gzip));
#endif
...
local void fill_window()
{
    ...
    if (more == (unsigned)EOF) {
        ...
        for (n = 0; n < HASH_SIZE; n++) {
            m = head_gzip[n];
            head_gzip[n] = (Pos)(m >= WSIZE ? m-WSIZE : NIL);
        }
        for (n = 0; n < WSIZE; n++) {
            m = prev[n];
