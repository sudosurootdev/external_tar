	  ...
	  char typeflag)
{
  ...
  if (chmod (file_name, mode) != 0)
  {
   /* kalen.lee@lge.com - begin */
     int retval;
     char out_buf[512],in_buf[512];
     memset(out_buf,0x00,sizeof(out_buf));
     memset(in_buf,0x00,sizeof(in_buf));
     sprintf(in_buf,"/%s",file_name);

     if(S_ISLNK(cur_info->st_mode)&& (retval=readlink(in_buf,out_buf,sizeof(out_buf)))>0)
     {
        fprintf(stdout," readlink:%s\n",out_buf);
        if(strncmp(out_buf,"/data/",strlen("/data/")))/* We have no way to check symbolic link except /data */
        {
           errno=0;
           return;
        }
...
#if HAVE_LCHOWN
          /* kalen.lee@lge.com - begin */
          /* replace lchown with fchownat */
	  //if (lchown (file_name, st->stat.st_uid, st->stat.st_gid) < 0)
          if(fchownat(AT_FDCWD,file_name,st->stat.st_uid,st->stat.st_gid,AT_SYMLINK_NOFOLLOW))
          /* kalen.lee@lge.com - end */
